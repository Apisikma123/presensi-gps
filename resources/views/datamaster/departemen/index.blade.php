    ...
    <div>
        <h5 class="mb-0 fw-bold text-dark d-flex align-items-center gap-2">
            ...
            <span class="badge bg-light text-dark font-mono" style="border: 1px solid #E2E8F0; font-size: 11px;">
                {{ $departemen->total() }} Total
            </span>
        </h5>
        <small class="text-muted" style="font-size: 12px;">Manajemen unit kerja, divisi operasional, kitchen, bar & service.</small>
    </div>
        ...
        <form action="{{ route('departemen.index') }}" method="GET">
            <div class="d-flex align-items-center gap-2">
                ...
                @if (Request('nama_dept'))
                    <a href="{{ route('departemen.index') }}" class="btn btn-outline-secondary d-flex align-items-center justify-content-center p-0 flex-shrink-0"
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
            ...
            <div class="text-muted" style="font-size: 12px;">
                @if ($departemen->total() > 0)
                    Menampilkan <span class="fw-bold text-dark font-mono">{{ $departemen->firstItem() }}</span> - <span class="fw-bold text-dark font-mono">{{ $departemen->lastItem() }}</span> dari <span class="fw-bold text-dark font-mono">{{ $departemen->total() }}</span> total departemen
                @else
                    Menampilkan 0 data
                @endif
            </div>
            ...
            @if ($departemen->lastPage() > 1)
                <div class="page-slider-container">
                    ...
                    <input type="range" class="page-slider-range" id="pageSlider"
                        min="1" max="{{ $departemen->lastPage() }}" value="{{ $departemen->currentPage() }}"
                        oninput="document.getElementById('sliderBadge').innerText = this.value"
                        onchange="navigatePage(this.value)">
                    <span class="badge bg-primary font-mono" id="sliderBadge">{{ $departemen->currentPage() }}</span>
                </div>
            @endif
