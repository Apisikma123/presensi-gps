    ...
    <div class="col-12">
        ...
        <div class="card admin-filter-toolbar mb-3">
            <div class="card-body p-3">
                <form action="{{ route('izinsakit.index') }}" method="GET">
                    <div class="row g-2 align-items-center">
                        ...
                        <div class="col-lg-2 col-md-4 col-12 d-flex align-items-end gap-1.5 pt-lg-4">
                            ...
                            @if (Request('dari') || Request('sampai') || Request('nama_karyawan') || Request('status') !== null || Request('kode_cabang'))
                                <a href="{{ route('izinsakit.index') }}" class="btn btn-outline-secondary d-inline-flex align-items-center justify-content-center gap-1" style="height: 38px; padding: 0 10px;" title="Reset Filter">
                        ...
                        @forelse ($izinsakit as $d)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if (!empty($d->doc_sid))
                                        <div class="mt-0.5">
                                            <a href="{{ url('storage/uploads/sid/'.$d->doc_sid) }}" target="_blank" class="badge bg-label-info text-decoration-none d-inline-flex align-items-center gap-1" style="font-size: 10px;">
                                ...
                                <td class="text-end">
                                    <div class="d-inline-flex align-items-center gap-1.5">
                                        @can('izinsakit.approve')
                                            @if ($d->status == 0)
                                                <button type="button" class="btnApprove" kode_izin_sakit="{{ Crypt::encrypt($d->kode_izin_sakit) }}" style="color: #059669; border-color: #A7F3D0;" title="Persetujuan Izin Sakit">
                                        ...
                                        @can('izinsakit.index')
                                            <button type="button" class="btnShow" kode_izin_sakit="{{ Crypt::encrypt($d->kode_izin_sakit) }}" title="Detail Izin Sakit">
                                        ...
                                        @can('izinsakit.edit')
                                            @if ($d->status == 0)
                                                <button type="button" class="btnEdit" kode_izin_sakit="{{ Crypt::encrypt($d->kode_izin_sakit) }}" title="Edit Izin Sakit">
                                        ...
                                        @can('izinsakit.delete')
                                            @if ($d->status == 0)
                                                <form method="POST" name="deleteform" class="deleteform d-inline m-0"
                                                    ...
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="delete-confirm" title="Hapus Izin Sakit">
