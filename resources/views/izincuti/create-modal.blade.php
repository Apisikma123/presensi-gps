<form action="{{ route('izincuti.store') }}" method="POST" id="formIzin" enctype="multipart/form-data">
    ...
    <div class="form-group mb-3">
        <label for="kode_cuti" class="form-label fw-semibold" style="font-size: 13px;">Pilih Jenis Cuti <span class="text-danger fw-bold ms-0.5">*</span></label>
        <select name="kode_cuti" id="kode_cuti" class="form-select" required>
    ...
    <x-input-with-icon icon="ti ti-briefcase" label="Pelimpahan Tugas" name="pelimpahan_tugas" optional="true" />
    <x-input-with-icon icon="ti ti-user" label="Nama Kepala Divisi" name="nama_kepala_divisi" optional="true" />
    <x-textarea-label label="Alasan Cuti" name="keterangan" required="true" />
    ...
    <div class="modal-footer-standard d-flex align-items-center justify-content-end gap-2 mt-4 pt-3 border-top">
        ...
        @else
            <a href="{{ route('izincuti.index') }}" class="btn btn-outline-secondary px-3">Kembali</a>
        @endif
