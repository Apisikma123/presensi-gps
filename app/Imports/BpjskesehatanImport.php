...
class BpjskesehatanImport implements ToCollection, WithStartRow
{
    ...
    public function collection(Collection $rows)
    {
        foreach ($rows as $row) {
            ...
            if (!$tanggal_berlaku) {
                Log::warning('Bpjskesehatan Import: Invalid date for NIK ' . $nik . ' - Value: ' . $tanggal_berlaku_raw);
                continue;
            }
            ...
            if (!$karyawan) {
                Log::warning('Bpjskesehatan Import: Employee not found with NIK ' . $nik);
                continue;
            }

            // Check if already exists for this NIK and date
            $exists = Bpjskesehatan::where('nik', $nik)
