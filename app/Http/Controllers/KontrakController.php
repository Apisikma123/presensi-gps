...
class KontrakController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('dokumen')) {
            ...
            $filename = 'kontrak_' . $data['nik'] . '_' . time() . '.' . $file->getClientOriginalExtension();
            $destinationPath = 'public/uploads/kontrak';
    ...
    public function update(Request $request, string $encryptedId)
    {
        ...
        if ($request->hasFile('dokumen')) {
            ...
            if ($kontrak->dokumen && Storage::exists('public/uploads/kontrak/' . $kontrak->dokumen)) {
                Storage::delete('public/uploads/kontrak/' . $kontrak->dokumen);
            }
    ...
    protected function prepareContractContent($kontrak, $tunjanganItems, $setting)
    {
        ...
        $placeholders = [
            '{{no_kontrak}}' => e($kontrak->no_kontrak),
            '{{no_dokumen}}' => e($kontrak->no_dokumen ?? '-'),
            '{{hari_ini}}' => e(now()->isoFormat('dddd')),
            '{{tanggal_hari_ini}}' => e(now()->isoFormat('D MMMM Y')),
            '{{nama_hrd}}' => e($setting->nama_hrd ?? 'Pihak Pertama'),
            '{{jabatan_hrd}}' => e('Owner ' . ($setting->nama_perusahaan ?? 'Perusahaan')),
            '{{nama_perusahaan}}' => e($setting->nama_perusahaan ?? 'Perusahaan'),
            '{{alamat_perusahaan}}' => e($setting->alamat ?? 'Lokasi Perusahaan'),
            '{{nama_karyawan}}' => e($kontrak->nama_karyawan),
            '{{tempat_lahir}}' => e($kontrak->tempat_lahir ?? '-'),
            '{{tanggal_lahir}}' => e($kontrak->tanggal_lahir ? Carbon::parse($kontrak->tanggal_lahir)->isoFormat('D MMMM Y') : '-'),
            '{{pendidikan_terakhir}}' => e($kontrak->pendidikan_terakhir ?? '-'),
            '{{jenis_kelamin}}' => e($kontrak->jenis_kelamin == 'L' ? 'Laki-laki' : ($kontrak->jenis_kelamin == 'P' ? 'Perempuan' : ($kontrak->jenis_kelamin ?? '-'))),
            '{{alamat_karyawan}}' => e($kontrak->alamat ?? '-'),
            '{{no_ktp}}' => e($kontrak->no_ktp ?? '-'),
            '{{no_hp}}' => e($kontrak->no_hp ?? '-'),
            '{{jabatan}}' => e($kontrak->nama_jabatan),
            '{{cabang}}' => e($kontrak->nama_cabang),
            '{{tanggal_mulai}}' => e($kontrak->dari ? Carbon::parse($kontrak->dari)->isoFormat('D MMMM Y') : '-'),
            '{{tanggal_selesai}}' => e($kontrak->sampai ? Carbon::parse($kontrak->sampai)->isoFormat('D MMMM Y') : '-'),
            '{{gaji_pokok}}' => e('Rp ' . number_format($kontrak->jumlah_gaji ?? 0, 0, ',', '.')),
            '{{total_gaji}}' => e('Rp ' . number_format($totalGaji, 0, ',', '.')),
        ];
